...
class SSD(nn.Module):
    def __init__(self, num_classes: int, base_net: nn.ModuleList, source_layer_indexes: List[int],
        ...
        # print(f'SSD device : {self.device}')
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for index, end_layer_index in enumerate(self.source_layer_indexes):
            ...
            if path:
                ...
                if path.s1<0:
                    ...
                    # print(f'PATH S1 :{path.s1}')
                    for id, layer in enumerate(sub[:path.s1]):
                        # print(f'sub[:path.s1] : {layer}')
                        # print(f'    x:{x.shape}')
                        x = layer(x)
                        # print(f'    x:{x.shape}')
                    y = x
        ...
        for layer in self.base_net[end_layer_index:]:
            # print(f'base_net[end_layer_index:] : {layer}')
            # print(f'    x:{x.shape}')
            x = layer(x)
            # print(f'    x:{x.shape}')
