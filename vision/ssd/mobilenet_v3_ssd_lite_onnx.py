...
from ..nn.mobilenet_v3_onnx import MobileNetV3
...
def create_mobilenetv3_ssd_lite(num_classes, model_mode="LARGE", width_mult=1.0, is_test=False, device="cpu"):
    base_net = MobileNetV3(model_mode=model_mode).features

    source_layer_indexes = [
        GraphPath(11, 'conv', -1),
        # 11,
        20,
    ] if model_mode=="SMALL" else [
        GraphPath(16, 'conv', -1),
        # 11,
        22,
    ]
    ...
    regression_headers = ModuleList([
        SeperableConv2d(in_channels=round(288 * width_mult) if model_mode=="SMALL" else round(672 * width_mult),
                        out_channels=6 * 4, kernel_size=3, padding=1, onnx_compatible=False),
...
def create_mobilenetv3_ssd_lite_predictor(net, candidate_size=200, nms_method=None, sigma=0.5, device=None):
    # print(f'create predoctor device condition : {device}')
