...
class SqueezeBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f'SqueezeBlock forward: batch-{batch}, channels-{channels}, height-{height}, width-{width}')
        # out = F.avg_pool2d(x, kernel_size=[height, width]).view(batch, -1)
        out = F.avg_pool2d(x, kernel_size=self.kernel_size).view(batch, -1)
        # print(f'DENSE before out-{out.shape}')
        out = self.dense(out)
        # print(f'DENSE after out-{out.shape}')
        out = out.view(batch, channels, 1, 1)
        # print(f'view out-{out.shape}')
        # out = hard_sigmoid(out)
        # print(f'RETURN out-{(out*x).shape}')
        return out * x
        # return out


def pass_SqueezeBlock(exp_size, out, wh=56):
    squeeze_block = SqueezeBlock(exp_size, wh)
    return squeeze_block(out)


class MobileBlock(nn.Module):
    # def __init__(self, in_channels, out_channels, kernal_size, stride, nonLinear, SE, exp_size, dropout_rate=1.0):
    def __init__(self, in_channels, out_channels, kernal_size, stride, nonLinear, SE, exp_size, se_wh):
        ...
        # print(f'in_channels-{in_channels}, exp_size-{exp_size}')
    ...
    def __init__(self, model_mode="LARGE", num_classes=30, multiplier=1.0, dropout_rate=0.0):
        ...
        if model_mode == "LARGE":
            ...
            for in_channels, out_channels, kernal_size, stride, nonlinear, se, exp_size, se_wh in layers:
                ...
                self.block.append(MobileBlock(in_channels, out_channels, kernal_size, stride, nonlinear, se, exp_size, se_wh))
                self.features.append(MobileBlock(in_channels, out_channels, kernal_size, stride, nonlinear, se, exp_size, se_wh))
            ...
            # in_channels, out_channels, kernal_size, stride, nonlinear, se, exp_size = [16, 16, 3, 2, "RE", True, 16]
            # in_channels = _make_divisible(in_channels * multiplier)
            # out_channels = _make_divisible(out_channels * multiplier)
            # exp_size = _make_divisible(exp_size * multiplier)
            # self.block = MobileBlock(in_channels, out_channels, kernal_size, stride, nonlinear, se, exp_size)
