...
parser = argparse.ArgumentParser(description="SSD Evaluation on SKU Dataset.")
...
if __name__ == '__main__':
    ...
    if args.dataset_type == "voc":
        ...
        dataset = SKUDataset(args.dataset, mode=1)
    ...
    if args.net == 'vgg16-ssd':
        ...
        net = create_mobilenetv2_ssd_lite(
            len(class_names), width_mult=args.mb2_width_mult, is_test=True, device=DEVICE)
    elif args.net == 'mb3-ssd-lite':
        net = create_mobilenetv3_ssd_lite(
            len(class_names), width_mult=args.mb3_width_mult, is_test=True, device=DEVICE)
    else:
    ...
    print(f'Eval ssd devive condition : {DEVICE}')
    net = net.to(DEVICE)
    ...
    # print(f'Average time {sum(load_time)/len(load_time)} to Load Image')
    # print(f'Average time {sum(predict_time)/len(predict_time)} to Predict')
